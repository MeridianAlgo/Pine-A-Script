...
function main() {
  ...
  pinescript.indicator("Mega Coverage Fixture", ({ overlay: true, max_bars_back: 500 }));
  ...
  const score = 0;
  arr = pinescript.arrayNew(5);
  pinescript.arraySet(arr, 0, 1);
  pinescript.arraySet(arr, 1, 2);
  pinescript.arraySet(arr, 2, 3);
  pinescript.arraySet(arr, 3, 4);
  pinescript.arraySet(arr, 4, 5);
  mx = pinescript.arrayMax(arr);
  idx = pinescript.arrayIndexOf(arr, mx);
  for (let i = 1; i <= len; i++) {
    {
      sigUp = pinescript.cross(src[i], src[(i + 1)]);
      sigDn = pinescript.cross(src[i], src[(i + 1)]);
      profit = (sigUp ? (src[i] - src[(i + 1)]) : (sigDn ? (src[(i + 1)] - src[i]) : 0));
      score = (score + profit);
    }
  }
  trend = ((src > pinescript.sma(src, len)) ? "Bull" : ((src < pinescript.sma(src, len)) ? "Bear" : "Flat"));
  baseColor = pinescript.color.rgb(32, 64, 160);
  col = (useGrad ? pinescript.color.new(baseColor, 10) : pinescript.color.new(pinescript.color.gray, 90));
  p = pinescript.plot(src, ({ color: col, title: "Src" }));
  p2 = pinescript.plot(pinescript.sma(src, len), ({ color: pinescript.color.new(pinescript.color.blue, 10), title: "MA" }));
  pinescript.fill(p, p2, ({ color: pinescript.color.new(pinescript.color.green, 80) }));
  buy = pinescript.cross(src, pinescript.sma(src, len));
  sell = pinescript.cross(src, pinescript.sma(src, len));
  pinescript.plotshape((buy ? src : null), ({ title: "Buy", style: pinescript.shape.triangleup, location: pinescript.location.belowbar, color: pinescript.color.green, size: pinescript.size.small }));
  pinescript.plotshape((sell ? src : null), ({ title: "Sell", style: pinescript.shape.triangledown, location: pinescript.location.abovebar, color: pinescript.color.red, size: pinescript.size.small }));
  if (((bar_index % 50) === 0)) {
    {
      const t = pinescript.table.new(pinescript.position.top_right, 2, 2, ({ border_width: 1 }));
      pinescript.table.cell(t, 0, 0, "Trend");
      pinescript.table.cell(t, 1, 0, trend);
      pinescript.table.cell(t, 0, 1, "Score");
      pinescript.table.cell(t, 1, 1, pinescript.strToString(score));
    }
  }
}
