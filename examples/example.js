...
function main() {
  ...
  pinescript.indicator("Quantum MA with Consolidation Zones [Q-Meridian]", ({ shorttitle: "Quantum MA [Q-Meridian]", overlay: true, max_bars_back: 1100 }));
  ...
  fixed_color = input.color(pinescript.color.rgb(0, 180, 140, 90), "Fixed Color", ({ group: group_color, tooltip: "Color used when gradient is disabled." }));
  gradient_start_color = input.color(pinescript.color.rgb(32, 64, 160), "Gradient Start (Bearish)", ({ group: group_color }));
  gradient_middle_color = input.color(pinescript.color.rgb(144, 10, 199), "Gradient Middle", ({ group: group_color }));
  gradient_end_color = input.color(pinescript.color.rgb(128, 192, 255), "Gradient End (Bullish)", ({ group: group_color }));
  cloud_bull_color = input.color(pinescript.color.rgb(0, 180, 141, 64), "Cloud Bullish", ({ group: group_color }));
  cloud_bear_color = input.color(pinescript.color.rgb(240, 80, 80, 64), "Cloud Bearish", ({ group: group_color }));
  ...
  table_bg_color = input.color(pinescript.color.rgb(15, 15, 23, 85), "Table Background", ({ group: group_table }));
  table_border_color = input.color(pinescript.color.rgb(30, 30, 45, 85), "Table Border", ({ group: group_table }));
  table_header_bg = input.color(pinescript.color.rgb(25, 25, 35, 95), "Header Background", ({ group: group_table }));
  table_header_text = input.color(pinescript.color.rgb(240, 240, 240), "Header Text", ({ group: group_table }));
  table_label_text = input.color(pinescript.color.rgb(180, 180, 200), "Label Text", ({ group: group_table }));
  table_value_text = input.color(pinescript.color.rgb(255, 255, 255), "Value Text", ({ group: group_table }));
  ...
  if (use_gradient) {
    {
      ...
      ma_color = pinescript.color.rgb(((pinescript.color.r(start_color) * (1 - weight)) + (pinescript.color.r(end_color) * weight)), ((pinescript.color.g(start_color) * (1 - weight)) + (pinescript.color.g(end_color) * weight)), ((pinescript.color.b(start_color) * (1 - weight)) + (pinescript.color.b(end_color) * weight)));
    }
  } else {
    {
  ...
  price_plot = pinescript.plot((show_cloud ? src : null), "Price", ({ color: pinescript.color.new(pinescript.color.gray, 100) }));
  pinescript.fill(price_plot, plot_ma, ({ color: ((src > best_ma) ? cloud_bull_color : cloud_bear_color) }));
  plot_size = ((signal_size === "Small") ? pinescript.size.small : ((signal_size === "Large") ? pinescript.size.large : pinescript.size.normal));
  pinescript.plotshape(((show_signals && buy_signal) ? best_ma : null), "Buy Signal", ({ style: ((signal_style === "Triangle") ? pinescript.shape.triangleup : ((signal_style === "Circle") ? pinescript.shape.circle : pinescript.shape.square)), location: pinescript.location.belowbar, color: pinescript.color.green, size: pinescript.size.normal }));
  pinescript.plotshape(((show_signals && sell_signal) ? best_ma : null), "Sell Signal", ({ style: ((signal_style === "Triangle") ? pinescript.shape.triangledown : ((signal_style === "Circle") ? pinescript.shape.circle : pinescript.shape.square)), location: pinescript.location.abovebar, color: pinescript.color.red, size: pinescript.size.normal }));
  if ((show_optimal_info && show_table)) {
    {
      font_size = ((table_size === "Small") ? pinescript.size.small : ((table_size === "Large") ? pinescript.size.large : pinescript.size.normal));
      const prev_price = 0;
      price_change = (((src - prev_price) / prev_price) * 100);
      prev_price = src;
      const table_pos = pinescript.position.top_right;
      if ((table_position === "Top Left")) {
        {
          table_pos = pinescript.position.top_left;
        }
      } else {
        if ((table_position === "Bottom Right")) {
          {
            table_pos = pinescript.position.bottom_right;
          }
        } else {
          if ((table_position === "Bottom Left")) {
            {
              table_pos = pinescript.position.bottom_left;
            }
          }
        }
      }
      const info_table = pinescript.table.new(table_pos, 2, (show_performance ? 8 : 7), ({ border_color: table_border_color, border_width: 1, bgcolor: pinescript.color.new(table_bg_color, table_transparency) }));
      pinescript.table.cell(info_table, 0, 0, "⚛ Quantum MA", ({ text_color: table_header_text, text_size: font_size, bgcolor: table_header_bg, text_halign: pinescript.text.align_center }));
      pinescript.table.cell(info_table, 1, 0, "Analysis", ({ text_color: table_header_text, text_size: font_size, bgcolor: table_header_bg, text_halign: pinescript.text.align_center }));
      pinescript.table.cell(info_table, 0, 1, "🌊 MA Type", ({ text_color: table_label_text, text_size: font_size }));
      pinescript.table.cell(info_table, 1, 1, best_ma_type, ({ text_color: table_value_text, text_size: font_size, bgcolor: pinescript.color.new(pinescript.color.rgb(40, 40, 60, 90), 0) }));
      pinescript.table.cell(info_table, 0, 2, "📏 Length", ({ text_color: table_label_text, text_size: font_size }));
      pinescript.table.cell(info_table, 1, 2, pinescript.strToString(best_length), ({ text_color: table_value_text, text_size: font_size, bgcolor: pinescript.color.new(pinescript.color.rgb(35, 35, 50, 90), 0) }));
      pinescript.table.cell(info_table, 0, 3, "🎯 Score", ({ text_color: table_label_text, text_size: font_size }));
      score_val = pinescript.arrayMax(final_scores);
      score_color = ((score_val > 0) ? pinescript.color.rgb(100, 255, 100) : ((score_val < 0) ? pinescript.color.rgb(255, 100, 100) : pinescript.color.rgb(200, 200, 200)));
      pinescript.table.cell(info_table, 1, 3, pinescript.strToString(score_val, "#.####"), ({ text_color: score_color, text_size: font_size, bgcolor: pinescript.color.new(pinescript.color.rgb(40, 40, 60, 90), 0) }));
      pinescript.table.cell(info_table, 0, 4, "📈 Trend", ({ text_color: table_label_text, text_size: font_size }));
      trend_color = ((trend_direction === "Bullish") ? pinescript.color.rgb(76, 175, 80) : ((trend_direction === "Bearish") ? pinescript.color.rgb(239, 83, 80) : pinescript.color.rgb(255, 235, 59)));
      pinescript.table.cell(info_table, 1, 4, trend_direction, ({ text_color: trend_color, text_size: font_size, bgcolor: pinescript.color.new(pinescript.color.rgb(35, 35, 50, 90), 0) }));
      pinescript.table.cell(info_table, 0, 5, "📊 R²", ({ text_color: table_label_text, text_size: font_size }));
      r2_color = ((r_squared_val > 0.7) ? pinescript.color.rgb(76, 175, 80) : ((r_squared_val > 0.4) ? pinescript.color.rgb(255, 193, 7) : pinescript.color.rgb(239, 83, 80)));
      pinescript.table.cell(info_table, 1, 5, pinescript.strToString(r_squared_val, "#.####"), ({ text_color: r2_color, text_size: font_size, bgcolor: pinescript.color.new(pinescript.color.rgb(40, 40, 60, 90), 0) }));
      pinescript.table.cell(info_table, 0, 6, "🚦 Signal", ({ text_color: table_label_text, text_size: font_size }));
      signal_icon = ((signal_status === "Buy") ? "🔼 " : ((signal_status === "Sell") ? "🔽 " : "◼ "));
      signal_color = ((signal_status === "Buy") ? pinescript.color.rgb(76, 175, 80) : ((signal_status === "Sell") ? pinescript.color.rgb(239, 83, 80) : pinescript.color.rgb(158, 158, 158)));
      pinescript.table.cell(info_table, 1, 6, (signal_icon + signal_status), ({ text_color: signal_color, text_size: font_size, bgcolor: pinescript.color.new(pinescript.color.rgb(35, 35, 50, 90), 0) }));
      if (show_performance) {
        {
          pinescript.table.cell(info_table, 0, 7, "💹 Change", ({ text_color: table_label_text, text_size: font_size }));
          change_color = ((price_change > 0) ? pinescript.color.rgb(76, 175, 80) : ((price_change < 0) ? pinescript.color.rgb(239, 83, 80) : pinescript.color.rgb(158, 158, 158)));
          change_icon = ((price_change > 0) ? "△ " : ((price_change < 0) ? "▽ " : "○ "));
          pinescript.table.cell(info_table, 1, 7, ((change_icon + pinescript.strToString(price_change, "#.##")) + "%"), ({ text_color: change_color, text_size: font_size, bgcolor: pinescript.color.new(pinescript.color.rgb(40, 40, 60, 90), 0) }));
        }
      }
    }
  }
