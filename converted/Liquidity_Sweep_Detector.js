...
function main() {
  ...
  let length = input.int(8, "Pivot Lookback", ({ minval: 3, maxval: 50, group: zoneGroup, tooltip: "Higher = fewer but more significant swing points. Lower = more zones and more sweep signals." }));
  let maxZones = input.int(8, "Max Active Zones", ({ minval: 2, maxval: 15, group: zoneGroup, tooltip: "How many unbroken zones to track per side" }));
  let zoneRange = input.string("Wick Rejection", "Zone Range", ({ options: ["Wick Rejection", "Full Bar"], group: zoneGroup }));
  let showInvalidated = input.bool(false, "Show Invalidated Zones (faded)", ({ group: zoneGroup }));
  let sweepGroup = "🎯  Sweep Detection";
  let showSweeps = input.bool(true, "Show Sweep Signals", ({ group: sweepGroup }));
  let showSweepBG = input.bool(true, "Highlight Sweep Bars", ({ group: sweepGroup }));
  let filterGroup = "🔬  Signal Quality Filters";
  let useTrendFilter = input.bool(false, "Trend Filter (EMA)", ({ group: filterGroup, tooltip: "Only show bull sweeps above EMA, bear sweeps below EMA. With-trend sweeps = institutional reloading." }));
  let trendEmaLen = input.int(50, "Trend EMA Length", ({ minval: 10, maxval: 200, group: filterGroup }));
  let useVolFilter = input.bool(false, "Volume Spike Filter", ({ group: filterGroup, tooltip: "Sweep bar must have higher-than-average volume. Real liquidity grabs leave a volume footprint." }));
  let volMultiplier = input.float(1.2, "Volume Multiplier", ({ minval: 1, maxval: 5, step: 0.1, group: filterGroup }));
  let volAvgLen = input.int(20, "Volume Avg Length", ({ minval: 5, maxval: 100, group: filterGroup }));
  let useWickFilter = input.bool(false, "Wick Rejection Filter", ({ group: filterGroup, tooltip: "Sweep bar must show strong rejection — big wick, small body relative to total range." }));
  let wickMinPct = input.float(40, "Min Wick %", ({ minval: 20, maxval: 90, step: 5, group: filterGroup, tooltip: "Minimum percentage of bar range that must be wick (rejection strength)" }));
  let vizGroup = "🎨  Visual Settings";
  let showDash = input.bool(true, "Show Dashboard", ({ group: vizGroup }));
  let dashPos = input.string("Top Right", "Dashboard Position", ({ options: ["Top Right", "Top Left", "Bottom Right", "Bottom Left"], group: vizGroup }));
  let dashSize = input.int(2, "Dashboard Size (1-3)", ({ minval: 1, maxval: 3, group: vizGroup }));
  let bullCol = input.color(pinescript.color.hex("#00b8d4"), "Support Zones", ({ group: vizGroup }));
  let bearCol = input.color(pinescript.color.hex("#ff6d00"), "Resistance Zones", ({ group: vizGroup }));
  let sweepCol = input.color(pinescript.color.hex("#e040fb"), "Sweep Signal", ({ group: vizGroup }));
  ...
  function addResistanceZone(top, btm, x) {
    ...
    pinescript.arrayPush(state.rz_box, box.new(x, top, x, btm, ({ border_color: pinescript.color.new(bearCol, 60), border_width: 1, bgcolor: pinescript.color.new(bearCol, 88), border_style: line.style_solid })));
  ...
  function addSupportZone(top, btm, x) {
    ...
    pinescript.arrayPush(state.sz_box, box.new(x, top, x, btm, ({ border_color: pinescript.color.new(bullCol, 60), border_width: 1, bgcolor: pinescript.color.new(bullCol, 88), border_style: line.style_solid })));
  ...
  alertcondition((state.bullSweepNow || state.bearSweepNow), "Any Sweep", "Sweep Detector: Liquidity sweep detected! Check chart for direction.");
}
