...
function main() {
  ...
  let basisLen = input.int(50, "Adaptive Baseline Length");
  let showBands = input.bool(true, "Show ATR Bands");
  let showBg = input.bool(true, "Background Shading");
  let showArrows = input.bool(true, "Show Entry Arrows");
  let smoothLen = input.int(8, "Channel Smoothing", ({ minval: 1 }));
  let baseColorIn = input.color(pinescript.color.new(pinescript.color.hex("#5f5f5f"), 0), "Baseline Color");
  let upColorIn = input.color(pinescript.color.new(pinescript.color.hex("#000000"), 0), "Upper Band Color");
  let dnColorIn = input.color(pinescript.color.new(pinescript.color.hex("#000000"), 0), "Lower Band Color");
  let fillColorIn = input.color(pinescript.color.new(pinescript.color.blue, 75), "Channel Fill Color");
  let atrLen = 14;
  let atrMult = 2;
  let rsiLen = 14;
  let macdFast = 12;
