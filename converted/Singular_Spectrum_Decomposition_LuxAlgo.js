...
function main() {
  ...
  var lagInput = pinescript.inputInt(20, "Window Length (L)", ({ minval: 2, tooltip: "The embedding window length (L). Larger values capture longer cycles." }));
  var trainingInput = pinescript.inputInt(100, "Buffer Length (N)", ({ minval: 50, tooltip: "The number of recent bars (N) used for the static decomposition." }));
  var forecastInput = pinescript.inputInt(20, "Forecast Length", ({ minval: 1, tooltip: "Number of bars to extrapolate into the future." }));
  var GROUP_STYLE = "Style";
  var showTrend = pinescript.inputBool(true, "Trend on Price", ({ inline: "Trend", group: GROUP_STYLE }));
  var trendColorInput = pinescript.inputColor(pinescript.color.hex("#ff5d00"), "", ({ inline: "Trend", group: GROUP_STYLE }));
  var showP1 = pinescript.inputBool(true, "Long Term Periodic", ({ inline: "P1", group: GROUP_STYLE }));
  var p1ColorInput = pinescript.inputColor(pinescript.color.hex("#2962ff"), "", ({ inline: "P1", group: GROUP_STYLE }));
  var showP2 = pinescript.inputBool(true, "Short Term Periodic", ({ inline: "P2", group: GROUP_STYLE }));
  var p2ColorInput = pinescript.inputColor(pinescript.color.hex("#f06292"), "", ({ inline: "P2", group: GROUP_STYLE }));
  var showNoise = pinescript.inputBool(true, "Noise", ({ inline: "Noise", group: GROUP_STYLE }));
  var noiseColorInput = pinescript.inputColor(pinescript.color.new(pinescript.color.hex("#9c9c9c"), 50), "", ({ inline: "Noise", group: GROUP_STYLE }));
  var showExtrapolation = pinescript.inputBool(true, "Show Extrapolation", ({ group: GROUP_STYLE, tooltip: "Show the forecast as dashed lines." }));
  var DATA = pinescript.color.hex("#DBDBDB");
  var HEADERS = pinescript.color.hex("#808080");
  ...
  var dashboardTooltip = "Enable or disable the dashboard.";
  var dashboardPositionTooltip = "Select the dashboard location.";
  var dashboardSizeTooltip = "Select the dashboard size.";
  var dashboardInput = pinescript.inputBool(true, "Dashboard", ({ group: DASHBOARD_GROUP, tooltip: dashboardTooltip }));
  ...
  function sortEigen(vectors, values) {
    var size_1 = values.size();
    var indices = pinescript.arrayNew(size_1);
    ...
    for (let i = 0; i <= (size_1 - 2); i++) {
      for (let j = 0; j <= ((size_1 - i) - 2); j++) {
        if ((values.get(indices.get(j)) < values.get(indices.get((j + 1))))) {
          var temp = indices.get(j);
          indices.set(j, indices.get((j + 1)));
          indices.set((j + 1), temp);
        }
      }
    }
    var sortedVectors = pinescript.matrixNew(vectors.rows(), vectors.columns());
    for (let col = 0; col <= (size_1 - 1); col++) {
      var oldCol = indices.get(col);
  ...
  function diagonalAveraging(V, k, y, L, N) {
    var K = ((N - L) + 1);
    var pc = pinescript.arrayNew(K, 0);
    for (let j = 0; j <= (K - 1); j++) {
      var val = 0;
      for (let i = 0; i <= (L - 1); i++) {
    ...
    var rc = pinescript.arrayNew(N, 0);
    for (let n = 0; n <= (N - 1); n++) {
      var sum = 0;
      var count = 0;
  ...
  function getLRR(V, indices, L) {
    var a = pinescript.arrayNew((L - 1), 0);
    var verticalSum = 0;
    for (const k of (indices ?? [])) {
      var pi_k = V.get((L - 1), k);
  ...
  function extrapolate(src, lrr, nForecast) {
    var L = (lrr.size() + 1);
    var res = pinescript.arrayCopy(src);
    var lastRecon = src.get((src.size() - 1));
    var f0 = 0;
    for (let j = 0; j <= (L - 2); j++) {
      f0 += (lrr.get(j) * src.get(((src.size() - L) + j)));
    }
    var adjusted = pinescript.arrayNew(0);
    adjusted.push(lastRecon);
    var runningVal = lastRecon;
    var prevRaw = f0;
  ...
  function createPolyline(values, startBar, c, width = 2, style = line.style_solid) {
    var points = pinescript.arrayNew(0);
    ...
    return pinescript.polylineNew(points, ({ line_color: c, line_width: width, line_style: style }));
  }
  function createOverlayPolyline(values, startBar, c, width = 2, style = line.style_solid) {
    ...
    return pinescript.polylineNew(points, ({ line_color: c, line_width: width, line_style: style, force_overlay: true }));
  }
  ...
  if (barstate.islast) {
    var startIdx = (bar_index - (trainingInput - 1));
    var foreIdx = bar_index;
    var y = pinescript.arrayNew(0);
    for (let i = 0; i <= (trainingInput - 1); i++) {
      pinescript.arrayPush(y, pinescript.hist(0, close, ((trainingInput - 1) - i)));
    }
    var S = pinescript.matrixNew(lagInput, lagInput, 0);
    var nRowsX = ((trainingInput - lagInput) + 1);
    ...
    var evals = matrix.eigenvalues(S);
    var evecs = matrix.eigenvectors(S);
    var sortedV = sortEigen(evecs, evals);
    var trendValues = diagonalAveraging(sortedV, 0, y, lagInput, trainingInput);
    var trendExtrap = extrapolate(trendValues, getLRR(sortedV, pinescript.array.from(0), lagInput), forecastInput);
    var rc1 = diagonalAveraging(sortedV, 1, y, lagInput, trainingInput);
    var rc2 = diagonalAveraging(sortedV, 2, y, lagInput, trainingInput);
    var p1Values = pinescript.arrayNew(trainingInput, 0);
    for (let i = 0; i <= (trainingInput - 1); i++) {
      pinescript.arraySet(p1Values, i, (pinescript.arrayGet(rc1, i) + pinescript.arrayGet(rc2, i)));
    }
    var rc3 = diagonalAveraging(sortedV, 3, y, lagInput, trainingInput);
    var rc4 = diagonalAveraging(sortedV, 4, y, lagInput, trainingInput);
    var p2Values = pinescript.arrayNew(trainingInput, 0);
    for (let i = 0; i <= (trainingInput - 1); i++) {
      pinescript.arraySet(p2Values, i, (pinescript.arrayGet(rc3, i) + pinescript.arrayGet(rc4, i)));
    }
    var noiseValues = pinescript.arrayNew(trainingInput, 0);
    if ((lagInput > 5)) {
      for (let k = 5; k <= (lagInput - 1); k++) {
        var rck = diagonalAveraging(sortedV, k, y, lagInput, trainingInput);
        for (let i = 0; i <= (trainingInput - 1); i++) {
          pinescript.arraySet(noiseValues, i, (pinescript.arrayGet(noiseValues, i) + pinescript.arrayGet(rck, i)));
        }
      }
    }
    pinescript.polylineDelete(state.trendLine);
    pinescript.polylineDelete(state.trendFore);
    pinescript.polylineDelete(state.p1Line);
    pinescript.polylineDelete(state.p2Line);
    pinescript.polylineDelete(state.noiseLine);
    if (showTrend) {
    ...
    if (dashboardInput) {
      var period1 = getPeriod(p1Values);
      var period2 = getPeriod(p2Values);
      state.t_able.clear(0, 0, 1, 4);
      state.t_able.merge_cells(0, 0, 1, 0);
