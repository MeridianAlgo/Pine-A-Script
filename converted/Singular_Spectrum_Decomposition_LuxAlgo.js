...
function main() {
  ...
  let lagInput = pinescript.inputInt(20, "Window Length (L)", ({ minval: 2, tooltip: "The embedding window length (L). Larger values capture longer cycles." }));
  let trainingInput = pinescript.inputInt(100, "Buffer Length (N)", ({ minval: 50, tooltip: "The number of recent bars (N) used for the static decomposition." }));
  let forecastInput = pinescript.inputInt(20, "Forecast Length", ({ minval: 1, tooltip: "Number of bars to extrapolate into the future." }));
  let GROUP_STYLE = "Style";
  let showTrend = pinescript.inputBool(true, "Trend on Price", ({ inline: "Trend", group: GROUP_STYLE }));
  let trendColorInput = pinescript.inputColor(pinescript.color.hex("#ff5d00"), "", ({ inline: "Trend", group: GROUP_STYLE }));
  let showP1 = pinescript.inputBool(true, "Long Term Periodic", ({ inline: "P1", group: GROUP_STYLE }));
  let p1ColorInput = pinescript.inputColor(pinescript.color.hex("#2962ff"), "", ({ inline: "P1", group: GROUP_STYLE }));
  let showP2 = pinescript.inputBool(true, "Short Term Periodic", ({ inline: "P2", group: GROUP_STYLE }));
  let p2ColorInput = pinescript.inputColor(pinescript.color.hex("#f06292"), "", ({ inline: "P2", group: GROUP_STYLE }));
  let showNoise = pinescript.inputBool(true, "Noise", ({ inline: "Noise", group: GROUP_STYLE }));
  let noiseColorInput = pinescript.inputColor(pinescript.color.new(pinescript.color.hex("#9c9c9c"), 50), "", ({ inline: "Noise", group: GROUP_STYLE }));
  let showExtrapolation = pinescript.inputBool(true, "Show Extrapolation", ({ group: GROUP_STYLE, tooltip: "Show the forecast as dashed lines." }));
  ...
  let dashboardInput = pinescript.inputBool(true, "Dashboard", ({ group: DASHBOARD_GROUP, tooltip: dashboardTooltip }));
  ...
  function extrapolate(src, lrr, nForecast) {
    let L = (lrr.size() + 1);
    let res = pinescript.arrayCopy(src);
