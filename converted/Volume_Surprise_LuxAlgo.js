...
function main() {
  ...
  let length = pinescript.inputInt(50, ({ minval: 1 }));
  let smooth = pinescript.inputInt(1, ({ minval: 1 }));
  let autoFormat = pinescript.input(true, "Auto Selection", ({ group: "Periods", tooltip: "Automatically choose a practical combination of periods based on the chart timeframe" }));
  let considerMinutes = pinescript.input(true, "Minutes", ({ group: "Periods" }));
  let considerHours = pinescript.input(true, "Hours", ({ group: "Periods" }));
  let considerDays = pinescript.input(false, "Days", ({ inline: "day", group: "Periods" }));
  let daysOptions = pinescript.inputString("Day of Week", "", ({ options: ["Day of Week", "Day of Month", "Day of Year"], inline: "day", group: "Periods" }));
  let considerMonths = pinescript.input(false, "Months", ({ group: "Periods" }));
  let considerQuarters = pinescript.input(false, "Quarters", ({ group: "Periods" }));
  let summaryMethod = pinescript.inputString("Mean", "Method", ({ options: ["Mean", "Percentile"], group: "Summary" }));
  let percentile = pinescript.input(50, ({ active: (summaryMethod === "Percentile"), group: "Summary" }));
  let showForecast = pinescript.input(true, "Show Forecast", ({ group: "Forecast" }));
  let forecastWindow = pinescript.inputInt(50, "Forecast Window", ({ minval: 1, maxval: 499, group: "Forecast" }));
  let forecastStyleColor = pinescript.input(pinescript.color.hex("#2962ff"), "Style", ({ inline: "forecast_style", group: "Forecast" }));
  let forecastStyleLine = pinescript.inputString("Dotted", "", ({ options: ["Solid", "Dashed", "Dotted"], inline: "forecast_style", group: "Forecast" }));
  ...
  if ((showForecast && barstate.islast)) {
    ...
    pinescript.polylineDelete(pinescript.hist(0, pinescript.polylineNew(forecastCoordinates, ({ xloc: xloc.bar_time, line_style: state.forecastStyle, line_color: forecastStyleColor })), 1));
  }
