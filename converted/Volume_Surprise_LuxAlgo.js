...
function main() {
  ...
  var length = pinescript.inputInt(50, ({ minval: 1 }));
  var smooth = pinescript.inputInt(1, ({ minval: 1 }));
  var autoFormat = pinescript.input(true, "Auto Selection", ({ group: "Periods", tooltip: "Automatically choose a practical combination of periods based on the chart timeframe" }));
  var considerMinutes = pinescript.input(true, "Minutes", ({ group: "Periods" }));
  var considerHours = pinescript.input(true, "Hours", ({ group: "Periods" }));
  var considerDays = pinescript.input(false, "Days", ({ inline: "day", group: "Periods" }));
  var daysOptions = pinescript.inputString("Day of Week", "", ({ options: ["Day of Week", "Day of Month", "Day of Year"], inline: "day", group: "Periods" }));
  var considerMonths = pinescript.input(false, "Months", ({ group: "Periods" }));
  var considerQuarters = pinescript.input(false, "Quarters", ({ group: "Periods" }));
  var summaryMethod = pinescript.inputString("Mean", "Method", ({ options: ["Mean", "Percentile"], group: "Summary" }));
  var percentile = pinescript.input(50, ({ active: (summaryMethod === "Percentile"), group: "Summary" }));
  var showForecast = pinescript.input(true, "Show Forecast", ({ group: "Forecast" }));
  var forecastWindow = pinescript.inputInt(50, "Forecast Window", ({ minval: 1, maxval: 499, group: "Forecast" }));
  var forecastStyleColor = pinescript.input(pinescript.color.hex("#2962ff"), "Style", ({ inline: "forecast_style", group: "Forecast" }));
  var forecastStyleLine = pinescript.inputString("Dotted", "", ({ options: ["Solid", "Dashed", "Dotted"], inline: "forecast_style", group: "Forecast" }));
  function getSummary(id, key) {
    id.get(key).v.percentile_linear_interpolation(percentile);
    id.get(key).v.avg();
    var summary = ((summaryMethod === "Mean") ? undefined : ((summaryMethod === "Percentile") ? undefined : null));
  }
  ...
  if (state.format === undefined) state.format = pinescript.strFormat("{0}{1}{2}{3}{4}ss", (considerQuarters ? pinescript.strToString(pinescript.ceil((month / 3))) : ""), (considerMonths ? "MM:" : ""), (considerDays ? ((daysOptions === "Day of Week") ? "EEE:" : ((daysOptions === "Day of Month") ? "dd:" : "DDD:")) : ""), (considerHours ? "hh:" : ""), (considerMinutes ? "mm:" : ""));
  if (state.data === undefined) state.data = pinescript.mapNew();
  var vol = pinescript.sma(pinescript.series(0, volume), smooth);
  var key = pinescript.str.format_time(time, state.format);
  if (!state.data.keys().includes(key)) {
    state.data.put(key, vector.new(pinescript.arrayNew(0)));
  }
  var summary = getSummary(state.data, key);
  state.data.get(key).v.push(vol);
  if ((state.data.get(key).v.size() > length)) {
    state.data.get(key).v.shift();
  }
  line.style_dotted;
  line.style_dashed;
  line.style_solid;
  ...
  if ((showForecast && barstate.islast)) {
    var forecastCoordinates = pinescript.arrayNew(0);
    forecastCoordinates.push(chart.point.from_time(time, summary));
    for (let i = 1; i <= forecastWindow; i++) {
      var t = pinescript.time(timeframe.period, ({ bars_back: -i }));
      var forecastKey = pinescript.str.format_time(t, state.format);
      if (state.data.keys().includes(forecastKey)) {
        var forecastValue = getSummary(state.data, forecastKey);
        forecastCoordinates.push(chart.point.from_time(t, forecastValue));
      } else {
        forecastCoordinates.push(chart.point.from_time(t, null));
      }
    }
    pinescript.polylineDelete(pinescript.hist(1, pinescript.polylineNew(forecastCoordinates, ({ xloc: xloc.bar_time, line_style: state.forecastStyle, line_color: forecastStyleColor })), 1));
  }
  var css = ((vol > summary) ? pinescript.color.new(pinescript.color.hex("#089981"), 50) : pinescript.color.new(pinescript.color.hex("#f23645"), 50));
  var plotVolume = pinescript.plot(vol, "Volume", ({ color: pinescript.color.gray }));
  var plotEv = pinescript.plot(summary, "Expected Volume", ({ color: pinescript.color.orange, linestyle: plot.linestyle_dotted }));
