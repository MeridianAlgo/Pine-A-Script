...
function main() {
  ...
  var fastLen = pinescript.inputInt(20, ({ title: "Fast MA Length" }));
  var slowLen = pinescript.inputInt(120, ({ title: "Slow MA Length" }));
  var maType = pinescript.inputString("SMA", ({ title: "Moving Average Type", options: ["SMA", "EMA", "WMA", "LMA"] }));
  var useADX = pinescript.inputBool(true, ({ title: "Enable ADX Filter" }));
  var adxLen = pinescript.inputInt(14, ({ title: "ADX Length" }));
  var adxThreshold = pinescript.inputFloat(20, ({ title: "ADX Threshold" }));
  function ma(src, len) {
    var out = null;
    if ((maType === "SMA")) {
      out = pinescript.sma(pinescript.series(0, src), len);
    } else {
      if ((maType === "EMA")) {
        out = pinescript.ema(pinescript.series(1, src), len);
      } else {
        if ((maType === "WMA")) {
          out = pinescript.wma(pinescript.series(2, src), len);
        } else {
          out = pinescript.rma(pinescript.series(3, src), len);
        }
      }
    }
    return out;
  }
  var fastMA = ma(close, fastLen);
  var slowMA = ma(close, slowLen);
  var longBias = (fastMA > slowMA);
  var shortBias = (fastMA < slowMA);
  var upMove = (high - pinescript.hist(4, high, 1));
  var downMove = (pinescript.hist(5, low, 1) - low);
  var plusDM = (((upMove > downMove) && (upMove > 0)) ? upMove : 0);
  var minusDM = (((downMove > upMove) && (downMove > 0)) ? downMove : 0);
  var trur = pinescript.rma(pinescript.series(6, pinescript.tr(true)), adxLen);
  var plusDI = ((100 * pinescript.rma(pinescript.series(7, plusDM), adxLen)) / trur);
  var minusDI = ((100 * pinescript.rma(pinescript.series(8, minusDM), adxLen)) / trur);
  var dx = ((100 * pinescript.abs((plusDI - minusDI))) / (plusDI + minusDI));
  var adx = pinescript.rma(pinescript.series(9, dx), adxLen);
  var adxOK = (!useADX || (adx >= adxThreshold));
  var enterLong = ((longBias && adxOK) && (pinescript.strategy.position_size <= 0));
  var enterShort = ((shortBias && adxOK) && (pinescript.strategy.position_size >= 0));
  if (enterLong) {
