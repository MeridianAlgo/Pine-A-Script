...
function main() {
  ...
  let fastLen = pinescript.inputInt(20, ({ title: "Fast MA Length" }));
  let slowLen = pinescript.inputInt(120, ({ title: "Slow MA Length" }));
  let maType = pinescript.inputString("SMA", ({ title: "Moving Average Type", options: ["SMA", "EMA", "WMA", "LMA"] }));
  let useADX = pinescript.inputBool(true, ({ title: "Enable ADX Filter" }));
  let adxLen = pinescript.inputInt(14, ({ title: "ADX Length" }));
  let adxThreshold = pinescript.inputFloat(20, ({ title: "ADX Threshold" }));
  function ma(src, len) {
    let out = null;
    if ((maType === "SMA")) {
  ...
  let upMove = (high - pinescript.hist(0, high, 1));
  let downMove = (pinescript.hist(1, low, 1) - low);
  let plusDM = (((upMove > downMove) && (upMove > 0)) ? upMove : 0);
  let minusDM = (((downMove > upMove) && (downMove > 0)) ? downMove : 0);
  let trur = pinescript.rma(pinescript.tr(true), adxLen);
