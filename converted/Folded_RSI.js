...
function main() {
  ...
  let cycle_len = input.int(40, "Cycle Window (corr length)", ({ group: grp_cycle, minval: 10, maxval: 600, tooltip: "Bars used to estimate correlation with a pure sine/cos basis." }));
  let target_period = input.int(20, "Target Sine Period (bars)", ({ group: grp_cycle, minval: 6, maxval: 300, tooltip: "Assumed dominant cycle length to correlate against." }));
  let use_harmonic2 = input.bool(true, "Also test 2× harmonic and keep the stronger", ({ group: grp_cycle }));
  let grp_map = "Correlation → RSI Length Mapping";
  let len_min = input.int(5, "RSI Length Min", ({ group: grp_map, minval: 2, maxval: 300 }));
  let len_max = input.int(33, "RSI Length Max", ({ group: grp_map, minval: 5, maxval: 600 }));
  let map_power = input.float(1, "Nonlinearity (power)", ({ group: grp_map, minval: 0.25, maxval: 4, tooltip: ">1 emphasizes strong correlations; <1 flattens the response." }));
  let invert_map = input.bool(false, "Invert mapping (strong cycle → longer length)", ({ group: grp_map }));
  let grp_src = "Source / Smoothing";
  let src = input.source(close, "Price Source", ({ group: grp_src }));
  let smooth_rsi = input.int(3, "Post Smoothing (EMA on RSI)", ({ group: grp_src, minval: 1, maxval: 50 }));
  ...
  function f_rsi_custom(_src, len_t) {
    let chg = (_src - pinescript.offset(_src, 1));
    ...
    state.avgU = (pinescript.na(pinescript.offset(state.avgU, 1)) ? up : (((1 - a) * pinescript.offset(state.avgU, 1)) + (a * up)));
    state.avgD = (pinescript.na(pinescript.offset(state.avgD, 1)) ? dn : (((1 - a) * pinescript.offset(state.avgD, 1)) + (a * dn)));
  ...
  function f_ema_custom(x, len) {
    ...
    state.s = (pinescript.na(pinescript.offset(state.s, 1)) ? x : (((1 - a) * pinescript.offset(state.s, 1)) + (a * x)));
    return state.s;
  }
  let ret = (math.log(src) - math.log(pinescript.offset(src, 1)));
