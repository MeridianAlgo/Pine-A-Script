...
function main() {
  ...
  let lenShortInput = input.int(13, "Short Term", ({ minval: 3, group: grpCore }));
  let lenLongInput = input.int(63, "Long Term", ({ minval: 20, group: grpCore }));
  let lenVolInput = input.int(50, "Volume Average (Logic)", ({ minval: 10, group: grpCore }));
  let sensitivity = 2;
  let grpTrend = "Trend Filter";
  let trendPenaltyWeight = input.float(1, "Trend Penalty Strength", ({ minval: 0, maxval: 5, step: 0.5, group: grpTrend }));
  let grpStruct = "Structure Settings";
  let hlLookbackInput = input.int(63, "Structure Lookback", ({ minval: 20, group: grpStruct }));
  let penaltyFactor = input.float(0.75, "Penalty for Lower Low", ({ minval: 0, maxval: 1, step: 0.05, group: grpStruct }));
  let grpBonus = "Consistency Weight";
  let bonusMax = input.int(15, "Max Consistency Points", ({ minval: 5, maxval: 30, group: grpBonus }));
  let scr_vol_len = input.int(50, "Avg Volume Length", ({ minval: 10, group: grpScreen }));
  let rs_benchmark = input.symbol("SPY", "RS Benchmark", ({ group: grpScreen }));
  let rs_len = input.int(21, "RS Period", ({ group: grpScreen }));
  let adr_len = input.int(20, "ADR Period", ({ group: grpScreen }));
  let grpColor = "Color & Thresholds";
  let thHigh = input.int(80, "Level 2 (High)", ({ minval: 60, maxval: 100, group: grpColor }));
  let thLow = input.int(60, "Level 1 (Base)", ({ minval: 0, maxval: 80, group: grpColor }));
  let cHigh = input.color(pinescript.color.hex("#00E676"), "Level 2 Color", ({ group: grpColor }));
  let cLow = input.color(pinescript.color.hex("#2962FF"), "Level 1 Color", ({ group: grpColor }));
  let cBase = input.color(pinescript.color.hex("#787B86"), "Base Color (<60)", ({ group: grpColor }));
  ...
  let netChange = pinescript.abs((close - pinescript.offset(close, lenShort)));
  let totalTravel = math.sum(ta.tr(true), lenShort);
  ...
  let lowRecent = pinescript.lowest(low, lenShort);
  ...
  let mCapBasic = request.financial(syminfo.tickerid, "MARKET_CAP_BASIC", "D", ({ ignore_invalid_symbol: true }));
  ...
  let rs_rank = ta.percentrank(rs_ratio, rs_len);
