...
function main() {
  ...
  let pivotLenInput = pinescript.inputInt(5, "Pivot Lookback", ({ minval: 2, group: OB_GROUP }));
  let maxOBsInput = pinescript.inputInt(5, "Max Unmitigated OBs", ({ minval: 1, maxval: 50, group: OB_GROUP }));
  let VIS_GROUP = "Visualization";
  let bullColorInput = pinescript.inputColor(pinescript.color.new(BULL_COLOR, 80), "Bullish OB Color", ({ group: VIS_GROUP }));
  let bearColorInput = pinescript.inputColor(pinescript.color.new(BEAR_COLOR, 80), "Bearish OB Color", ({ group: VIS_GROUP }));
  let hideOverlappedInput = pinescript.inputBool(true, "Hide Overlapped Zones", ({ group: VIS_GROUP }));
  let showLabelsInput = pinescript.inputBool(true, "Show Strength Labels", ({ group: VIS_GROUP }));
  let showStrongestInput = pinescript.inputBool(true, "Show Strongest OB Plot", ({ group: VIS_GROUP }));
  let bufferSizeInput = pinescript.inputInt(5, "Strongest OB Buffer Size", ({ minval: 1, maxval: 100, group: VIS_GROUP }));
  ...
  let bullBOS = pinescript.crossover(pinescript.series(0, close), pinescript.series(1, state.lastHi));
  let bearBOS = pinescript.crossunder(pinescript.series(2, close), pinescript.series(3, state.lastLo));
  if ((bullBOS || bearBOS)) {
    ...
    if (bullBOS) {
      ...
      for (let i = 1; i <= searchLen; i++) {
        if ((pinescript.hist(4, close, i) < pinescript.hist(5, open, i))) {
          if ((pinescript.na(minLow) || (pinescript.hist(6, low, i) < minLow))) {
            minLow = pinescript.hist(7, low, i);
            obTop = pinescript.hist(8, high, i);
            obBtm = pinescript.hist(9, low, i);
            obIdx = i;
            obVol = pinescript.hist(10, volume, i);
          }
        }
      }
      state.lastHi = null;
      ...
      let maxHigh = null;
      for (let i = 1; i <= searchLen; i++) {
        if ((pinescript.hist(11, close, i) > pinescript.hist(12, open, i))) {
          if ((pinescript.na(maxHigh) || (pinescript.hist(13, high, i) > maxHigh))) {
            maxHigh = pinescript.hist(14, high, i);
            obTop = pinescript.hist(15, high, i);
            obBtm = pinescript.hist(16, low, i);
            obIdx = i;
            obVol = pinescript.hist(17, volume, i);
          }
        }
      }
      state.lastLo = null;
    ...
    if (!pinescript.na(obIdx)) {
      ...
      let avgVol = pinescript.hist(18, volSMA, obIdx);
      let strength = calculateStrength(obHeight, breakoutDist, (obVol / avgVol));
      let newOB = OrderBlock.new(obTop, obBtm, pinescript.hist(19, time, obIdx), strength, bullBOS, null, null, false);
  ...
  let hasChanged = (strongestAvgPrice !== pinescript.hist(20, strongestAvgPrice, 1));
