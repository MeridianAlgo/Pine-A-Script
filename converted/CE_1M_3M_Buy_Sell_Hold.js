...
function main() {
  ...
  function f_ce_dir(_length, _mult, _useClose) {
    var atr = (_mult * pinescript.atr(_length));
    var longStop = ((_useClose ? pinescript.highest(pinescript.series(0, close), _length) : pinescript.highest(pinescript.series(1, high), _length)) - atr);
    var longStopPrev = pinescript.nz(pinescript.hist(2, longStop, 1), longStop);
    longStop = ((pinescript.hist(3, close, 1) > longStopPrev) ? pinescript.max(longStop, longStopPrev) : longStop);
    var shortStop = ((_useClose ? pinescript.lowest(pinescript.series(4, close), _length) : pinescript.lowest(pinescript.series(5, low), _length)) + atr);
    var shortStopPrev = pinescript.nz(pinescript.hist(6, shortStop, 1), shortStop);
  ...
  var dir_1m = f_ce_dir(length, mult, useClose);
  var dir_3m = pinescript.requestSecurity(syminfo.tickerid, "3", f_ce_dir(length, mult, useClose), barmerge.gaps_off, barmerge.lookahead_off);
  var state_1 = 0;
  state_1 = (((dir_3m === 1) && (dir_1m === 1)) ? 1 : (((dir_3m === -1) && (dir_1m === -1)) ? -1 : 0));
  var buySignal = ((state_1 === 1) && (pinescript.hist(8, state_1, 1) !== 1));
  var sellSignal = ((state_1 === -1) && (pinescript.hist(9, state_1, 1) !== -1));
  var holdSignal = ((state_1 === 0) && (pinescript.hist(10, state_1, 1) !== 0));
