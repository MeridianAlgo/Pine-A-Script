...
function main() {
  ...
  null;
  // Study: CE 1M + 3M Buy, Sell, Hold
  // Options: {"overlay":true}
  let length = 7;
  let mult = 2;
  let useClose = true;
  function f_ce_dir(_length, _mult, _useClose) {
    let atr = (_mult * pinescript.atr(_length));
    let longStop = ((_useClose ? pinescript.highest(pinescript.series(0, close), _length) : pinescript.highest(pinescript.series(1, high), _length)) - atr);
    let longStopPrev = pinescript.nz(pinescript.hist(2, longStop, 1), longStop);
    longStop = ((pinescript.hist(3, close, 1) > longStopPrev) ? pinescript.max(longStop, longStopPrev) : longStop);
    let shortStop = ((_useClose ? pinescript.lowest(pinescript.series(4, close), _length) : pinescript.lowest(pinescript.series(5, low), _length)) + atr);
    let shortStopPrev = pinescript.nz(pinescript.hist(6, shortStop, 1), shortStop);
    shortStop = ((pinescript.hist(7, close, 1) < shortStopPrev) ? pinescript.min(shortStop, shortStopPrev) : shortStop);
    if (state.dir === undefined) state.dir = 1;
    state.dir = ((close > shortStopPrev) ? 1 : ((close < longStopPrev) ? -1 : state.dir));
    return state.dir;
  }
  let dir_1m = f_ce_dir(length, mult, useClose);
  let dir_3m = pinescript.requestSecurity(syminfo.tickerid, "3", f_ce_dir(length, mult, useClose), barmerge.gaps_off, barmerge.lookahead_off);
  let state_1 = 0;
  state_1 = (((dir_3m === 1) && (dir_1m === 1)) ? 1 : (((dir_3m === -1) && (dir_1m === -1)) ? -1 : 0));
  let buySignal = ((state_1 === 1) && (pinescript.hist(8, state_1, 1) !== 1));
  let sellSignal = ((state_1 === -1) && (pinescript.hist(9, state_1, 1) !== -1));
  let holdSignal = ((state_1 === 0) && (pinescript.hist(10, state_1, 1) !== 0));
  pinescript.plotshape(buySignal, ({ title: "BUY", location: pinescript.location.belowbar, color: pinescript.color.green, style: pinescript.shape.labelup, text: "BUY" }));
  pinescript.plotshape(sellSignal, ({ title: "SELL", location: pinescript.location.abovebar, color: pinescript.color.red, style: pinescript.shape.labeldown, text: "SELL" }));
  pinescript.plotshape(holdSignal, ({ title: "HOLD", location: pinescript.location.bottom, color: pinescript.color.yellow, style: pinescript.shape.labelup, text: "HOLD" }));
}
