...
function main() {
  ...
  function f_ce_dir(_length, _mult, _useClose) {
    let atr = (_mult * ta.atr(_length));
    let longStop = ((_useClose ? pinescript.highest(close, _length) : pinescript.highest(high, _length)) - atr);
    let longStopPrev = pinescript.nz(pinescript.offset(longStop, 1), longStop);
    longStop = ((pinescript.offset(close, 1) > longStopPrev) ? pinescript.max(longStop, longStopPrev) : longStop);
    let shortStop = ((_useClose ? pinescript.lowest(close, _length) : pinescript.lowest(low, _length)) + atr);
    let shortStopPrev = pinescript.nz(pinescript.offset(shortStop, 1), shortStop);
    shortStop = ((pinescript.offset(close, 1) < shortStopPrev) ? pinescript.min(shortStop, shortStopPrev) : shortStop);
  ...
  let buySignal = ((state_1 === 1) && (pinescript.offset(state_1, 1) !== 1));
  let sellSignal = ((state_1 === -1) && (pinescript.offset(state_1, 1) !== -1));
  let holdSignal = ((state_1 === 0) && (pinescript.offset(state_1, 1) !== 0));
