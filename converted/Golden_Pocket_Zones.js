...
const ZoneData = { new: function(upper, lower, startBar, age, strength) { return { upper: upper, lower: lower, startBar: startBar, age: age, strength: strength }; } };
...
function main() {
  ...
  let lookbackDays = input.int(20, "Lookback Days", ({ minval: 1, maxval: 100, tooltip: "Number of daily candles to analyze for golden pocket zones" }));
  let zoneHeightPct = input.float(3, "Zone Height (% of ADR10)", ({ minval: 0.1, maxval: 50, step: 0.1, tooltip: "Height of golden pocket zone as percentage of ADR10 - SAME for all zones" }));
  let minCandleSizePct = input.float(60, "Min Candle Size (% of ADR10)", ({ minval: 0, maxval: 200, step: 5, tooltip: "Only show zones for candles larger than this % of ADR10. Set to 0 to show all zones." }));
  let showLabels = input.bool(true, "Show Labels", ({ tooltip: "Display age and strength labels inside zones" }));
  let zoneColor = input.color(pinescript.color.new(pinescript.color.hex("#FFB84D"), 20), "Zone Color", ({ tooltip: "Color of the golden pocket zones" }));
  let textColor = input.color(pinescript.color.new(pinescript.color.hex("#000000"), 0), "Text Color", ({ tooltip: "Color of the text labels" }));
  let [dailyHigh, dailyLow, dailyTime] = pinescript.requestSecurity(syminfo.tickerid, "D", [high, low, time], ({ lookahead: barmerge.lookahead_off }));
  ...
  if (state.dailyLows === undefined) state.dailyLows = pinescript.arrayNew();
  if (state.dailyBars === undefined) state.dailyBars = pinescript.arrayNew();
  if (state.dailyADRs === undefined) state.dailyADRs = pinescript.arrayNew();
  if (state.zones === undefined) state.zones = pinescript.arrayNew();
  if (state.currentADR10 === undefined) state.currentADR10 = null;
  function f_getPips() {
    return ((syminfo.type === "forex") ? ((syminfo.currency === "JPY") ? 0.01 : 0.0001) : (((syminfo.ticker === "XAUUSD") || (syminfo.ticker === "GOLD")) ? 0.01 : 0.0001));
  }
  function f_toPips(val) {
  ...
  function mergeZones(upper1, lower1, upper2, lower2) {
