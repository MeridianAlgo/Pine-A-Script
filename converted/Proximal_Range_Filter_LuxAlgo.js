...
function main() {
  ...
  null;
  // Study: L1 Proximal Filter [LuxAlgo]
  // Options: {"overlay":true}
  var BULL_COLOR = pinescript.color.hex("#089981");
  var BEAR_COLOR = pinescript.color.hex("#f23645");
  var srcInput = pinescript.inputSource(close, "Source");
  var atrMultInput = pinescript.inputFloat(1.5, "ATR Multiplier", ({ minval: 0, step: 0.1, tooltip: "Defines the noise threshold by multiplying the 200-period ATR." }));
  var muInput = pinescript.inputFloat(0.6, "Adaptation Rate (μ)", ({ minval: 0, maxval: 1, step: 0.1, tooltip: "Controls how fast the filter adapts to price changes." }));
  var threshold = (pinescript.atr(200) * atrMultInput);
  if (state.z === undefined) state.z = null;
  if (state.v === undefined) state.v = 0;
  if ((bar_index === 0)) {
    state.z = srcInput;
  } else {
    var zPrev = pinescript.hist(0, state.z, 1);
    var vPrev = pinescript.hist(1, state.v, 1);
    var zPred = (zPrev + vPrev);
    var zTemp = (zPred + (muInput * (srcInput - zPred)));
    var diff = (zTemp - zPrev);
    if ((pinescript.abs(diff) > threshold)) {
      state.v = (pinescript.sign(diff) * (pinescript.abs(diff) - threshold));
    } else {
      state.v = 0;
    }
  ...
  if ((state.z > pinescript.hist(2, state.z, 1))) {
    state.trend = 1;
  } else {
    if ((state.z < pinescript.hist(3, state.z, 1))) {
      state.trend = -1;
    }
  }
  var upChange = ((state.trend === 1) && (pinescript.hist(4, state.trend, 1) === -1));
  var downChange = ((state.trend === -1) && (pinescript.hist(5, state.trend, 1) === 1));
  var zColor = ((state.trend === 1) ? BULL_COLOR : BEAR_COLOR);
  var zPlot = pinescript.plot(state.z, "L1 Proximal Filter", ({ color: zColor, linewidth: 3 }));
  var srcPlot = pinescript.plot(srcInput, "Source Plot", ({ color: null }));
  var showFill = (((state.trend === 1) && (srcInput > state.z)) || ((state.trend === -1) && (srcInput < state.z)));
  var fillTopColor = (showFill ? pinescript.color.new(zColor, 50) : null);
  var fillBottomColor = (showFill ? pinescript.color.new(zColor, 100) : null);
  pinescript.fill(srcPlot, zPlot, srcInput, state.z, fillTopColor, fillBottomColor, "Trend Fill");
  var switchVal = ((upChange || downChange) ? pinescript.hist(6, state.z, 1) : null);
  var switchColor = (upChange ? BULL_COLOR : BEAR_COLOR);
  pinescript.plot(switchVal, "Trend Switch Dot", ({ color: switchColor, style: plot.style_circles, linewidth: 4, offset: -1 }));
}
