...
function main() {
  ...
  let srcInput = pinescript.inputSource(close, "Source");
  let atrMultInput = pinescript.inputFloat(1.5, "ATR Multiplier", ({ minval: 0, step: 0.1, tooltip: "Defines the noise threshold by multiplying the 200-period ATR." }));
  let muInput = pinescript.inputFloat(0.6, "Adaptation Rate (μ)", ({ minval: 0, maxval: 1, step: 0.1, tooltip: "Controls how fast the filter adapts to price changes." }));
  let threshold = (pinescript.atr(200) * atrMultInput);
  if (state.z === undefined) state.z = null;
  if (state.v === undefined) state.v = 0;
  if ((bar_index === 0)) {
    state.z = srcInput;
  } else {
    let zPrev = pinescript.hist(0, state.z, 1);
    let vPrev = pinescript.hist(1, state.v, 1);
  ...
  let upChange = ((state.trend === 1) && (pinescript.hist(4, state.trend, 1) === -1));
  let downChange = ((state.trend === -1) && (pinescript.hist(5, state.trend, 1) === 1));
  let zColor = ((state.trend === 1) ? BULL_COLOR : BEAR_COLOR);
  let zPlot = pinescript.plot(state.z, "L1 Proximal Filter", ({ color: zColor, linewidth: 3 }));
  let srcPlot = pinescript.plot(srcInput, "Source Plot", ({ color: null }));
  let showFill = (((state.trend === 1) && (srcInput > state.z)) || ((state.trend === -1) && (srcInput < state.z)));
  let fillTopColor = (showFill ? pinescript.color.new(zColor, 50) : null);
  let fillBottomColor = (showFill ? pinescript.color.new(zColor, 100) : null);
  pinescript.fill(srcPlot, zPlot, srcInput, state.z, fillTopColor, fillBottomColor, "Trend Fill");
  let switchVal = ((upChange || downChange) ? pinescript.hist(6, state.z, 1) : null);
  let switchColor = (upChange ? BULL_COLOR : BEAR_COLOR);
