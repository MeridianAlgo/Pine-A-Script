...
function main() {
  ...
  let dataType = input.string("Volume", ({ title: "Data Type", options: ["Volume", "Dollars", "Delta"] }));
  let priceCalc = input.string("Close", ({ title: "Price Calculation (for Dollars)", options: ["Close", "Open", "(Open + Close) / 2", "(High + Low + Open + Close) / 4", "(High + Low) / 2"] }));
