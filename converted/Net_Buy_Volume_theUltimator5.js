...
function main() {
  ...
  let dataType = pinescript.inputString("Volume", ({ title: "Data Type", options: ["Volume", "Dollars", "Delta"] }));
  let priceCalc = pinescript.inputString("Close", ({ title: "Price Calculation (for Dollars)", options: ["Close", "Open", "(Open + Close) / 2", "(High + Low + Open + Close) / 4", "(High + Low) / 2"] }));
  let fp = request.footprint(10000, 100);
  let buy = null;
  let sell = null;
  let fpChange = 0;
  let net = 0;
