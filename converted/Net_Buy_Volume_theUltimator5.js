...
function main() {
  ...
  null;
  // Study: Net Buy Volume [theUltimator5]
  // Options: {"overlay":false,"format":null}
  var dataType = pinescript.inputString("Volume", ({ title: "Data Type", options: ["Volume", "Dollars", "Delta"] }));
  var priceCalc = pinescript.inputString("Close", ({ title: "Price Calculation (for Dollars)", options: ["Close", "Open", "(Open + Close) / 2", "(High + Low + Open + Close) / 4", "(High + Low) / 2"] }));
  var fp = request.footprint(10000, 100);
  var buy = null;
  var sell = null;
  var fpChange = 0;
  var net = 0;
  ...
  open;
  close;
  var price = ((priceCalc === "Close") ? undefined : ((priceCalc === "Open") ? undefined : ((priceCalc === "(Open + Close) / 2") ? undefined : ((priceCalc === "(High + Low + Open + Close) / 4") ? undefined : ((priceCalc === "(High + Low) / 2") ? undefined : undefined)))));
  if ((bar_index === 1)) {
    net = 0;
    netDollars = 0;
  } else {
    net = (pinescript.hist(0, net, 1) + fpChange);
    netDollars = (pinescript.hist(1, netDollars, 1) + (fpChange * price));
  }
  var deltaColor = ((fpChange >= 0) ? pinescript.color.green : pinescript.color.red);
  pinescript.plot(((dataType === "Volume") ? net : null), "Net Vol", ({ color: pinescript.color.aqua, style: plot.style_stepline, linewidth: 2 }));
  pinescript.plot(((dataType === "Dollars") ? netDollars : null), "Net $", ({ color: pinescript.color.hex("#ef78b7"), style: plot.style_stepline, linewidth: 2 }));
  pinescript.plot(((dataType === "Delta") ? fpChange : null), "Delta", ({ color: deltaColor, style: plot.style_columns, linewidth: 2 }));
  pinescript.hline(((dataType === "Delta") ? 0 : null), "Zero Line", pinescript.color.gray, ({ linestyle: hline.style_solid }));
}
