...
function main() {
  ...
  let fastMinInput = input.int(5, "Fast Min Length", ({ minval: 2, group: GRID_GRP }));
  let fastMaxInput = input.int(25, "Fast Max Length", ({ minval: 2, group: GRID_GRP }));
  let fastStepInput = input.int(5, "Fast Step", ({ minval: 1, group: GRID_GRP }));
  let slowMinInput = input.int(10, "Slow Min Length", ({ minval: 2, group: GRID_GRP }));
  let slowMaxInput = input.int(50, "Slow Max Length", ({ minval: 2, group: GRID_GRP }));
  let slowStepInput = input.int(10, "Slow Step", ({ minval: 1, group: GRID_GRP }));
  let PROJ_GRP = "3D Projection Settings";
  let yOffsetInput = input.float(0, "Y Baseline Price", ({ group: PROJ_GRP }));
  let scaleXInput = input.float(3, "X Spacing (bars)", ({ group: PROJ_GRP }));
  let scaleZInput = input.float(5, "Z Height (Relative to Y Spacing)", ({ group: PROJ_GRP }));
  let STYLE_GRP = "Style";
  let bullColorInput = input.color(pinescript.color.hex("#089981"), "Bullish Color", ({ group: STYLE_GRP }));
  let bearColorInput = input.color(pinescript.color.hex("#f23645"), "Bearish Color", ({ group: STYLE_GRP }));
  let gridColorInput = input.color(pinescript.color.hex("#808080"), "Grid Color", ({ group: STYLE_GRP }));
  ...
  if (state.linesArray === undefined) state.linesArray = pinescript.arrayNew();
