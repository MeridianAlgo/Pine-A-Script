...
function main() {
  ...
  null;
  // Study: LuckyFX RBtrader strategy
  // Options: {"overlay":true}
  var enable_prev_candles = pinescript.inputBool(true, "Check: Prev Candles Same Dir?");
  var same_prev_candles = pinescript.inputInt(2, "Prev Candles Same (0–4)", ({ minval: 0, maxval: 4 }));
  var enable_body_percent = pinescript.inputBool(true, "Check: Cur > % Prev Body?");
  var body_percent_min = pinescript.inputFloat(0.3, "Body % Min (0.3)", ({ minval: 0.3, maxval: 2, step: 0.05 }));
  var body_percent_max = pinescript.inputFloat(3, "Body % Max (3.0)", ({ minval: 0.3, maxval: 3, step: 0.05 }));
  var ignore_doji = pinescript.inputBool(true, "Ignore if Prev is Doji");
  var enable_mean_range = pinescript.inputBool(true, "Check: Cur >= Mean Full Candle?");
  var mean_length = pinescript.inputInt(5, "Mean Candle Range (2-50)", ({ minval: 2, maxval: 50 }));
  var ignore_engulf = pinescript.inputBool(true, "Ignore if Engulfing (Body Size)");
  var enable_wick_compare = pinescript.inputBool(false, "Check: Cur wick > Prev wick (on/off)");
  var prev_open = pinescript.hist(0, open, 1);
  var prev_close = pinescript.hist(1, close, 1);
  var prev_high = pinescript.hist(2, high, 1);
  var prev_low = pinescript.hist(3, low, 1);
  var prev_body = pinescript.abs((prev_close - prev_open));
  var prev_upper_wick = (prev_high - pinescript.max(prev_open, prev_close));
  var prev_lower_wick = (pinescript.min(prev_open, prev_close) - prev_low);
  var prev_total_wick = (prev_upper_wick + prev_lower_wick);
  var prev_is_doji = (prev_total_wick >= (2 * prev_body));
  var prev_is_bull = (prev_close > prev_open);
  var prev_is_bear = (prev_close < prev_open);
  var cur_body = pinescript.abs((close - open));
  var cur_range = (high - low);
  var is_bull = (close > open);
  var is_bear = (close < open);
  var is_bullish_engulf = (cur_body > (prev_body + prev_upper_wick));
  var is_bearish_engulf = (cur_body > (prev_body + prev_lower_wick));
  var cur_upper_wick = (high - pinescript.max(open, close));
  var cur_lower_wick = (pinescript.min(open, close) - low);
  var sum_range = 0;
  for (let i = 1; i <= mean_length; i++) {
    sum_range = (sum_range + (pinescript.hist(4, high, i) - pinescript.hist(5, low, i)));
  }
  var mean_prev_range = (sum_range / mean_length);
  var is_cur_range_big_enough = (!enable_mean_range || (cur_range >= mean_prev_range));
  var prev_dir = (prev_is_bull ? 1 : (prev_is_bear ? -1 : 0));
  var match = true;
  if ((same_prev_candles > 0)) {
    for (let i = 2; i <= (same_prev_candles + 1); i++) {
      match = (match && (((pinescript.hist(6, close, i) > pinescript.hist(7, open, i)) ? 1 : ((pinescript.hist(8, close, i) < pinescript.hist(9, open, i)) ? -1 : 0)) === prev_dir));
    }
  }
  var last_n_same_as_prev = (!enable_prev_candles || ((prev_dir !== 0) && match));
  var body_percent_ok = (!enable_body_percent || ((cur_body > (body_percent_min * prev_body)) && (cur_body < (body_percent_max * prev_body))));
  var wick_condition = (!enable_wick_compare || ((is_bull && (cur_lower_wick > prev_lower_wick)) || (is_bear && (cur_upper_wick > prev_upper_wick))));
  var bull_bigger_than_bear = (((((((is_bull && prev_is_bear) && body_percent_ok) && (!ignore_doji || !prev_is_doji)) && (!ignore_engulf || !is_bullish_engulf)) && last_n_same_as_prev) && is_cur_range_big_enough) && wick_condition);
  var bear_bigger_than_bull = (((((((is_bear && prev_is_bull) && body_percent_ok) && (!ignore_doji || !prev_is_doji)) && (!ignore_engulf || !is_bearish_engulf)) && last_n_same_as_prev) && is_cur_range_big_enough) && wick_condition);
  pinescript.plotshape(bull_bigger_than_bear, ({ style: pinescript.shape.triangleup, color: pinescript.color.rgb(38, 129, 11), size: pinescript.size.small, location: pinescript.location.belowbar, title: "Bullish Signal" }));
  pinescript.plotshape(bear_bigger_than_bull, ({ style: pinescript.shape.triangledown, color: pinescript.color.rgb(226, 10, 10), size: pinescript.size.small, location: pinescript.location.abovebar, title: "Bearish Signal" }));
  pinescript.alertcondition(bull_bigger_than_bear, ({ title: "Bullish Signal", message: "Bullish > Bearish Setup" }));
  pinescript.alertcondition(bear_bigger_than_bull, ({ title: "Bearish Signal", message: "Bearish > Bullish Setup" }));
}
