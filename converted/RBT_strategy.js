...
function main() {
  ...
  let enable_prev_candles = input.bool(true, "Check: Prev Candles Same Dir?");
  let same_prev_candles = input.int(2, "Prev Candles Same (0–4)", ({ minval: 0, maxval: 4 }));
  let enable_body_percent = input.bool(true, "Check: Cur > % Prev Body?");
  let body_percent_min = input.float(0.3, "Body % Min (0.3)", ({ minval: 0.3, maxval: 2, step: 0.05 }));
  let body_percent_max = input.float(3, "Body % Max (3.0)", ({ minval: 0.3, maxval: 3, step: 0.05 }));
  let ignore_doji = input.bool(true, "Ignore if Prev is Doji");
  let enable_mean_range = input.bool(true, "Check: Cur >= Mean Full Candle?");
  let mean_length = input.int(5, "Mean Candle Range (2-50)", ({ minval: 2, maxval: 50 }));
  let ignore_engulf = input.bool(true, "Ignore if Engulfing (Body Size)");
  let enable_wick_compare = input.bool(false, "Check: Cur wick > Prev wick (on/off)");
  let prev_open = pinescript.offset(open, 1);
  let prev_close = pinescript.offset(close, 1);
  let prev_high = pinescript.offset(high, 1);
  let prev_low = pinescript.offset(low, 1);
