...
function main() {
  ...
  let enable_prev_candles = pinescript.inputBool(true, "Check: Prev Candles Same Dir?");
  let same_prev_candles = pinescript.inputInt(2, "Prev Candles Same (0–4)", ({ minval: 0, maxval: 4 }));
  let enable_body_percent = pinescript.inputBool(true, "Check: Cur > % Prev Body?");
  let body_percent_min = pinescript.inputFloat(0.3, "Body % Min (0.3)", ({ minval: 0.3, maxval: 2, step: 0.05 }));
  let body_percent_max = pinescript.inputFloat(3, "Body % Max (3.0)", ({ minval: 0.3, maxval: 3, step: 0.05 }));
  let ignore_doji = pinescript.inputBool(true, "Ignore if Prev is Doji");
  let enable_mean_range = pinescript.inputBool(true, "Check: Cur >= Mean Full Candle?");
  let mean_length = pinescript.inputInt(5, "Mean Candle Range (2-50)", ({ minval: 2, maxval: 50 }));
  let ignore_engulf = pinescript.inputBool(true, "Ignore if Engulfing (Body Size)");
  let enable_wick_compare = pinescript.inputBool(false, "Check: Cur wick > Prev wick (on/off)");
  let prev_open = pinescript.hist(0, open, 1);
  let prev_close = pinescript.hist(1, close, 1);
  let prev_high = pinescript.hist(2, high, 1);
  let prev_low = pinescript.hist(3, low, 1);
  let prev_body = pinescript.abs((prev_close - prev_open));
  let prev_upper_wick = (prev_high - pinescript.max(prev_open, prev_close));
  let prev_lower_wick = (pinescript.min(prev_open, prev_close) - prev_low);
  let prev_total_wick = (prev_upper_wick + prev_lower_wick);
  let prev_is_doji = (prev_total_wick >= (2 * prev_body));
  let prev_is_bull = (prev_close > prev_open);
  let prev_is_bear = (prev_close < prev_open);
  let cur_body = pinescript.abs((close - open));
  let cur_range = (high - low);
  let is_bull = (close > open);
  let is_bear = pinescript.close((prev_body + prev_upper_wick));
  let is_bearish_engulf = (cur_body > (prev_body + prev_lower_wick));
  let cur_upper_wick = (high - pinescript.max(open, close));
  ...
  let body_percent_ok = (!enable_body_percent || ((cur_body > (body_percent_min * prev_body)) && (cur_body < (body_percent_max * prev_body))));
  let wick_condition = (!enable_wick_compare || ((is_bull && (cur_lower_wick > prev_lower_wick)) || (is_bear && (cur_upper_wick > prev_upper_wick))));
  let bull_bigger_than_bear = (((((((is_bull && prev_is_bear) && body_percent_ok) && (!ignore_doji || !prev_is_doji)) && (!ignore_engulf || !is_bullish_engulf)) && last_n_same_as_prev) && is_cur_range_big_enough) && wick_condition);
  let bear_bigger_than_bull = (((((((is_bear && prev_is_bull) && body_percent_ok) && (!ignore_doji || !prev_is_doji)) && (!ignore_engulf || !is_bearish_engulf)) && last_n_same_as_prev) && is_cur_range_big_enough) && wick_condition);
