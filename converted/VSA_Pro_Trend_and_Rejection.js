...
function main() {
  ...
  let st_period = input.int(10, "ST Periyodu", ({ group: group_trend }));
  let st_multiplier = input.float(3, "ST Çarpanı", ({ step: 0.1, group: group_trend }));
  let group_volatility = "Hacim & Volatilite (VSA)";
  let atr_length = input.int(14, "ATR Uzunluğu", ({ group: group_volatility }));
  let min_vol_mult = input.float(1, "Min. Hacim Eşiği (Ortalama)", ({ group: group_volatility }));
  let effort_mult = input.float(2, "Yüksek Hacim (Görsel Ekleme)", ({ step: 0.1, group: group_volatility }));
  let result_mult = input.float(1.5, "Gövde Boyu (Görsel Momentum)", ({ step: 0.1, group: group_volatility }));
  let group_labels = "Etiket İsimleri";
  let txt_start_long = input.string("AL", "Trend Başlangıcı (Long)", ({ group: group_labels }));
  let txt_start_short = input.string("NAKİT", "Trend Bitişi (Sat)", ({ group: group_labels }));
  let [superTrendVal, superTrendDir] = ta.supertrend(st_multiplier, st_period);
  let isTrendNewGreen = ((superTrendDir === -1) && (pinescript.offset(superTrendDir, 1) === 1));
  let isTrendNewRed = ((superTrendDir === 1) && (pinescript.offset(superTrendDir, 1) === -1));
  let isGreenZone = (superTrendDir === -1);
  let isRedZone = (superTrendDir === 1);
  let atr = ta.atr(atr_length);
